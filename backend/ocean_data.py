...
def get_tide_data(station_id: str, date: str = None, days: int = 1) -> dict:
    ...
    try:
        ...
        response = requests.get(NOAA_TIDES_BASE_URL, params=params, timeout=10)
        ...
        if "predictions" in data:
            ...
            for prediction in data["predictions"]:
                tides.append({
                    "time": prediction["t"],
                    "height_feet": float(prediction["v"]),
                    "type": prediction["type"]  # "H" = high, "L" = low
                })
        ...
        print(f"Unexpected error in get_tide_data: {e}")
        return {"error": str(e), "station_id": station_id}

# MARINE WEATHER - Open-Meteo Marine API 
...
def get_marine_weather(latitude: float, longitude: float, days: int = 3) -> dict:
    ...
    try:
        ...
        print(f"Unexpected error in get_marine_weather: {e}")
        return {"error": str(e)}

# WATER TEMPERATURE - Open-Meteo Marine API 
def get_water_temperature(latitude: float, longitude: float, days: int = 7) -> dict:
    ...
    try:
        ...
        print(f"Unexpected error in get_water_temperature: {e}")
        return {"error": str(e)}

# LOCATION QUERIES

# basic NOAA tide stations
TIDE_STATIONS = {
    # California
    "san_diego": "9410230",
    "la_jolla": "9410230",
...
def generate_conditions_summary(data: dict) -> dict:
    ...
    try:
        ...
        if "marine_weather" in data:
            ...
            if wave_height < 0.5:
                summary["overall_rating"] = "calm"
                summary["best_for"].append("swimming")
                summary["best_for"].append("paddleboarding")
            elif wave_height < 1.5:
                summary["overall_rating"] = "moderate"
                summary["best_for"].append("surfing")
                summary["best_for"].append("kayaking")
            elif wave_height < 3.0:
                summary["overall_rating"] = "active"
                summary["best_for"].append("experienced surfing")
                summary["warnings"].append("Strong waves - be cautious!")
            else:
                summary["overall_rating"] = "rough"
                summary["warnings"].append("High surf conditions - experienced swimmers only")
